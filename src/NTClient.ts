...
export class NTClient extends Listenable<NTStatus> {
    ...
    getRobotStatus(): RobotStatus | null {
        ...
        return {
            nt: this.status,
            driveTrain:{
                rightsideturn: this.status.table['/SmartDashboard/Front Right Motor Position']?.value as number,
                leftsideturn: this.status.table['/SmartDashboard/Front Left Motor Position']?.value as number
            },
            hood: {
